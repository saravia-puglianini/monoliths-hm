...
XpraClient.prototype._keyb_process = function(pressed, event) {
	...
	// Chrome reports Spanish dead keys as the generic string "Dead".
	// Xpra 3.0.3 does not translate that value, so the following vowel is
	// sent without its accent.  Preserve the Spanish physical-key mapping.
	if (str=="Dead" && this._get_keyboard_layout()=="es") {
		if (keyname=="Quote") {
			keyname = event.shiftKey ? "dead_diaeresis" : "dead_acute";
		}
...
XpraClient.prototype._make_hello = function() {
	...
	this._update_capabilities({
		// Xpra 3.0.3 crashes its refresh callback when packet options are null.
		// RGB is lossless, so no automatic lossless refresh is needed.
		...
		// Disable the buggy in-place scroll decoder in this 3.0.x client.
		"encoding.scrolling"		: false,
...
XpraClient.prototype._new_window = function(wid, x, y, w, h, metadata, override_redirect, client_properties) {
	// Xpra 3.0.x can reuse a window id during Electron window recreation.
	// If the matching lost-window packet races with the new-window packet, the
	// old DIV remains in #screen and produces a second, stale "ghost" canvas.
	var previous = this.id_to_window[wid];
	if (previous) {
		previous.destroy();
		delete this.id_to_window[wid];
	}
	var stale_nodes = document.querySelectorAll('#screen > div[id="' + String(wid) + '"]');
	for (var stale_index = 0; stale_index < stale_nodes.length; stale_index++) {
		stale_nodes[stale_index].remove();
	}
...
XpraClient.prototype._process_lost_window = function(packet, ctx) {
	...
	// Be defensive about duplicate nodes left behind by an id-reuse race.
	var stale_nodes = document.querySelectorAll('#screen > div[id="' + String(wid) + '"]');
	for (var stale_index = 0; stale_index < stale_nodes.length; stale_index++) {
		stale_nodes[stale_index].remove();
	}
